...
export class HttpRequest implements IRequest {
  ...
  async get<T>(path: string, params: object | undefined, options: IRequestOptions = {}) {
    ...
    const response: AxiosResponse<T> = await this._httpClient.get(_url, _options);
  ...
  async post<T>(path: string, body: object, options: IRequestOptions) {
    ...
    const _options = this.getOptions(options);
    const response: AxiosResponse<T> = await this._httpClient.post(_url, body, _options);
  ...
  async put<T>(path: string, params: object, options: IRequestOptions) {
    const _url = options.isFullPath ? path : appContants.primaryEndPoint + path;
    const _options = this.getOptions(options);
    const response: AxiosResponse<T> = await this._httpClient.put(_url, params, _options);
    return response.data;
  }
  ...
  async patch<T>(path: string, params: object, options: IRequestOptions) {
    const _url = options.isFullPath ? path : appContants.primaryEndPoint + path;
    const _options = this.getOptions(options);
    const response: AxiosResponse<T> = await this._httpClient.patch(_url, params, _options);
    return response.data;
  }
  ...
  async delete<T>(path: string, params: object, options: IRequestOptions) {
    ...
    const response: AxiosResponse<T> = await this._httpClient.delete(_url, _options);
    return response.data;
  }

  async upload<T>(path: string, files: File, options: IRequestOptions, onProgress: () => void) {
    const _url = options.isFullPath ? path : appContants.primaryEndPoint + path;
    ...
    const response: AxiosResponse<T> = await this._httpClient.post(_url, _form, _options);
    return response.data;
  }
}
